...
int main(int argc, char **argv)
{
    ...
    cout << "     Here We Begin to Synthesize the T-Theta Section" << endl;
    ...
    //Theta-T道集加密
    cout << "     Here We Encrypted Theta-T gathers" << endl;
    arma::Mat<float> seis_thetat_after_nmo_interp;
    {
        ...
        interp2(x, y, seis_thetat, xi, yi, seis_thetat_after_nmo_interp);
    }

    int ntheta_interp = seis_thetat_after_nmo_interp.n_cols;
    int nt_interp = seis_thetat_after_nmo_interp.n_rows;
    ...
    for (int it = 0; it < nt; it++)
    {
        for (int ix = 0; ix < nx; ix++)
        {
            ...
            if (1)
            {
                //给定 x t0，确定theta
                float t0_temp = it * dt;
                float theta_temp = atan(x_temp * 1.0 / (v * t0_temp));
                        ...
                        t_coordinate + 1 < nt && t_coordinate >= 0)
                {
                    ...
                    seis_x_theta_xt_after_nmo(t_coordinate, ix) =
                        a_t *
                        (a_x * seis_thetat_after_nmo_interp(t_coordinate_interp, theta_coordinate) +
                         b_x * seis_thetat_after_nmo_interp(t_coordinate_interp, theta_coordinate + 1)) +
                        b_t *
                        (a_x * seis_thetat_after_nmo_interp(t_coordinate_interp + 1, theta_coordinate) +
                         b_x * seis_thetat_after_nmo_interp(t_coordinate_interp + 1, theta_coordinate + 1));
                }
            }
        }
    }
