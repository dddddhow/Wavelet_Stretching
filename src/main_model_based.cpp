...
int main(int argc, char **argv)
{
    ...
    arma::Mat<float> v_rms(nvz,nvx,fill::zeros);
    model.load("../data/model_nz400_nx2000.dat",raw_binary);
    ...
    float h2=300;
    float ttt=0;
    for(int iz=0; iz<h2; iz++)
    {
        ttt=ttt+dz*1.0/model(iz,0);
    }
    cout<<ttt<<endl;
    ttt=0;
    ttt=h2*dz*1.0/v_rms(h2,0);
    cout<<ttt<<endl;
...
*/
    ...
    for(int ivx=0; ivx<nvx; ivx++)
    {
        for(int iz=0; iz<ref_num(ivx);iz++)
        {
            cout<<ref_location(iz,ivx)<<endl;
        }
    }
